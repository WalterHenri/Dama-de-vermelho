...
class Bot:
    ...
    def choose_action(self, state, actions):
        ...
        if random.uniform(0, 1) < self.epsilon:
            ...
            q_values = [self.get_q_value(state, action.to_string()) for action in actions]
            max_q = max(q_values)
            max_actions = [actions[i] for i in range(len(actions)) if q_values[i] == max_q]
            return random.choice(max_actions)

    def update_q_value(self, state, action, reward, next_state, next_actions):
        old_value = self.get_q_value(state, action.to_string())
        future_rewards = [self.get_q_value(next_state, a.to_string()) for a in next_actions] if next_actions else [0]
        best_future = max(future_rewards)
        new_value = (1 - self.alpha) * old_value + self.alpha * (reward + self.gamma * best_future)
        self.q_table[(state, action.to_string())] = new_value
