...
fonte_tituloGG = pygame.font.Font('Fonts/Nightcore Demo.ttf', 100)
...
def desenharTexto(texto, fonte, cor, superficie, x, y, largura=0, altura=0, larguraMaxima=1000):
    ...
    for i, line in enumerate(lines):
        ...
        if largura > 0 and altura > 0:
            line_rect.topleft = (
            (largura - line_rect.width) / 2 + x, (altura - line_rect.height) / 2 + y + i * line_rect.height)
        else:
...
# desenha um botao, havendo a possibilidade de por uma borda arredondada, os dois ultimos parametros só devem ser
# informados caso o texto do botao deva ficar centralizado
